...
from vehiculos.models import Vehiculo, DocumentoVehiculo, Siniestro, RegistroAcceso
...
def dashboard_guardia(request):
    ...
    # Registros de entradas realizados por este guardia
    registros_entradas = RegistroAcceso.objects.filter(
        guardia=request.user,
        tipo_movimiento='entrada'
    ).order_by('-fecha_hora')

    # Estadísticas basadas en registros de acceso
    ...
    estadisticas = {
        'total_ingresados': registros_entradas.count(),
        'ingresos_hoy': registros_entradas.filter(fecha_hora__gte=inicio_hoy, fecha_hora__lte=fin_hoy).count(),
        'ingresos_semana': registros_entradas.filter(
            fecha_hora__date__gte=hoy_local - timezone.timedelta(days=7)
        ).count(),
        'en_taller_actual': Vehiculo.objects.exclude(estado__in=['entregado', 'pendiente', 'rechazado']).count(),
    }

    # Últimos ingresos
    ultimos_ingresos = registros_entradas[:10]
...
def dashboard_jefe_taller(request):
    ...
    # 4. Productividad por mecánico (últimos 30 días)
    from django.utils import timezone
