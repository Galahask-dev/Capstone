            ...
            if mecanico_id and request.user.rol in ['jefe_taller', 'admin']:
                ...
                try:
                    ...
                    Notificacion.objects.create(
                        usuario=mecanico,
                        mensaje=f"Te han asignado el vehículo {vehiculo.patente}",
                        link=f"/vehiculos/detalle/{vehiculo.id}/"
                    )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if citas_pendientes.exists():
                ...
                # ---------------------------------------------------------
                # AUTO-OT: Crear Tarea automáticamente basada en la cita
                # ---------------------------------------------------------
                try:
                    cita = citas_pendientes.first()
                    Tarea.objects.create(
                        vehiculo=vehiculo,
                        titulo=f"OT Automática: {cita.tipo_servicio}",
                        descripcion=f"Generada automáticamente al ingreso. Cita original: {cita.descripcion}",
                        prioridad='media',
                        estado='pendiente',
                        tiempo_estimado=cita.duracion_minutos
                    )
                    messages.info(request, '✅ Orden de Trabajo (OT) generada automáticamente.')
                except Exception as e:
                    messages.warning(request, f'Error al generar OT automática: {str(e)}')
                # ---------------------------------------------------------
                ...
                # ---------------------------------------------------------
                # AUTO-OT: Crear Tarea genérica si no hay cita (Opcional, según flujo)
                # ---------------------------------------------------------
                # Si se desea crear OT siempre, descomentar:
                # Tarea.objects.create(
                #     vehiculo=vehiculo,
                #     titulo="Ingreso sin Cita",
                #     descripcion="Vehículo ingresado sin cita previa. Diagnóstico inicial requerido.",
                #     prioridad='media',
                #     estado='pendiente'
                # )
                # messages.info(request, '✅ Orden de Trabajo (OT) generada automáticamente.')
