...
def agendar_cita(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            total_mecanicos = Usuario.objects.filter(rol='mecanico', is_active=True).count()
            
            citas_activas_horario = CitaMantenimiento.objects.filter(
                estado__in=['pendiente', 'confirmada'],
                fecha_hora__date=inicio_cita.date()
            ).count()
            
            if total_mecanicos > 0 and citas_activas_horario >= total_mecanicos:
                messages.warning(
                    request,
                    f'⚠️ ADVERTENCIA: El taller tiene {total_mecanicos} mecánicos disponibles y ya hay {citas_activas_horario} citas programadas para ese día. '
                    f'Considera elegir otro horario para mejor atención.'
                )
