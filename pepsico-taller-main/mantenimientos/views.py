...
@login_required
def crear_tarea_desde_cita(request, cita_id):
    if not (request.user.rol in ['jefe_taller', 'admin'] or request.user.is_superuser):
    ...
    if not vehiculo:
        try:
            vehiculo = Vehiculo.objects.get(patente=cita.patente)
        except Vehiculo.DoesNotExist:
            messages.error(request, 'Vehículo no encontrado para esta cita.')
            return redirect('dashboard_jefe_taller')

    tarea = Tarea.objects.create(
        vehiculo=vehiculo,
        titulo=f"Mantención: {cita.get_tipo_mantencion_display()}",
        descripcion=cita.observaciones or 'Generada desde cita de mantención',
        prioridad='media',
        estado='en_proceso',
        mecanico_asignado=mecanico,
        tiempo_estimado=cita.duracion_minutos or 60,
    )

    # Actualizar estado del vehículo a 'reparacion' y asignar mecánico
    vehiculo.mecanico_asignado = mecanico
    if vehiculo.estado != 'reparacion':
    ...
    messages.success(request, f'Tarea #{tarea.id} creada y asignada a {mecanico.get_full_name() or mecanico.username}.')
    # Volver a la página origen
    return redirect(request.META.get('HTTP_REFERER', 'clinica_dashboard'))
...
def completar_tarea(request, tarea_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Quitar asignación del mecánico al completar
            tarea.mecanico_asignado = None
            tarea.save()
            # Si no quedan tareas activas para el vehículo, marcar como 'listo'
            try:
                vehiculo = tarea.vehiculo
                quedan_activas = Tarea.objects.filter(vehiculo=vehiculo).exclude(estado='completada').exists()
                if not quedan_activas and vehiculo and vehiculo.estado != 'listo':
                    vehiculo.estado = 'listo'
                    vehiculo.fecha_salida = timezone.now()
                    vehiculo.mecanico_asignado = None
                    vehiculo.save()
                    messages.info(request, f'Vehículo {vehiculo.patente} marcado como "Listo para Retiro".')
            except Exception:
                pass
