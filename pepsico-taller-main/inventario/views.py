...
# ---------------------------------------------------------
# BODEGUERO FLOW VIEWS
# ---------------------------------------------------------
from mantenimientos.models import SolicitudRepuesto

@login_required
def gestionar_solicitudes(request):
    """Vista para que el bodeguero gestione solicitudes de repuestos"""
    if request.user.rol not in ['admin', 'jefe_taller']:
        messages.error(request, 'No tienes permisos para gestionar solicitudes de bodega.')
        return redirect('clinica_dashboard')

    solicitudes_pendientes = SolicitudRepuesto.objects.filter(estado='pendiente').select_related('repuesto', 'tarea', 'solicitante').order_by('fecha_solicitud')
    solicitudes_historial = SolicitudRepuesto.objects.exclude(estado='pendiente').select_related('repuesto', 'tarea', 'solicitante', 'respuesta_por').order_by('-fecha_respuesta')[:50]

    return render(request, 'inventario/gestionar_solicitudes.html', {
...
def aprobar_solicitud(request, solicitud_id):
    solicitud = get_object_or_404(SolicitudRepuesto, id=solicitud_id)
    ...
    if repuesto.stock_actual < solicitud.cantidad:
        messages.error(request, f'Stock insuficiente para aprobar. Stock: {repuesto.stock_actual}, Solicitado: {solicitud.cantidad}')
        return redirect('gestionar_solicitudes')
        
    # Aprobar y descontar stock
    solicitud.estado = 'aprobada'
    solicitud.fecha_respuesta = timezone.now()
    solicitud.respuesta_por = request.user
    solicitud.save()
    
    # Crear movimiento de inventario (real consumption)
    MovimientoInventario.objects.create(
        repuesto=repuesto,
        tipo_movimiento='salida',
        ...
        motivo=f"Solicitud aprobada por {request.user.username} para tarea {solicitud.tarea.id}",
        stock_anterior=repuesto.stock_actual,
        stock_posterior=repuesto.stock_actual - solicitud.cantidad
    )
    ...
    Notificacion.objects.create(
        usuario=solicitud.solicitante,
        mensaje=f"✅ Tu solicitud de {solicitud.cantidad} x {repuesto.nombre} fue APROBADA.",
        link=f"/mantenimientos/tarea/{solicitud.tarea.id}/repuestos/"
    )
    
    messages.success(request, 'Solicitud aprobada y stock descontado.')
    return redirect('gestionar_solicitudes')
...
def rechazar_solicitud(request, solicitud_id):
    ...
    Notificacion.objects.create(
        usuario=solicitud.solicitante,
        mensaje=f"❌ Tu solicitud de {solicitud.repuesto.nombre} fue RECHAZADA.",
        link=f"/mantenimientos/tarea/{solicitud.tarea.id}/repuestos/"
    )
